...
function validateRequest(
  requestBody: Partial<CreateUserRequest>,
): string | null {
  ...
  if (!requestBody.displayName?.trim()) {
    return 'יש להזין שם תצוגה.';
  }

  if (
    !requestBody.role ||
    !allowedRoles.has(requestBody.role)
  ) {
...
Deno.serve(async (request: Request) => {
  ...
  let createdAuthUserId: string | null =
    null;
