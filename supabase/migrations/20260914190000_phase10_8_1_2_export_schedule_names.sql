-- Phase 10.8.1.2 - Excel export: schedule names and complete parallel shifts
-- Extends the management calendar payload with profiles.schedule_name so export
-- can use the scheduling-facing name while keeping display_name as fallback.

begin;

-- Phase 10.5.12 · Correct management calendar time direction and premium resolution.
-- Keeps the Phase 10.5.11 logical slot de-duplication, while resolving the effective
-- day-kind before selecting a role template. This prevents a Saturday/Motzei Shabbat slot
-- from inheriting the premium settings of an equal-time Friday template.
