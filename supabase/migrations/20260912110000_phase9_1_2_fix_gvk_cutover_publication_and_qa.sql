...
-- Phase 9.1.2
-- 1) Repair a GVK cutover edge case: if the target job type already had an
--    availability period for the same month, the adapter reused that period.
--    Its source stayed unchanged, so the old publisher filter
--    (dap.source='legacy_cutover') skipped the migration draft even though the
--    draft itself was marked as legacy_cutover.
-- 2) Tighten QA so archived/history assignments to users who later became
--    inactive do not block a current cutover.

do $$
declare
  v_run public.gvk_legacy_migration_runs%rowtype;
  v_actor uuid;
...
begin
  ...
  v_actor := v_run.created_by;

  for v_map in
    select * from (values
      ('dispatcher'::text, v_run.dispatcher_job_type_id),
      ('on_call'::text, v_run.on_call_job_type_id),
      ('morning_driver'::text, v_run.morning_driver_job_type_id)
    ) x(source_kind, job_type_id)
  loop
    for v_draft in
      select distinct on (d.year,d.month)
        d.id as draft_id,
        d.availability_period_id,
        d.year,
        d.month,
        d.metrics,
        dap.config_snapshot
      ...
      where d.job_type_id=v_map.job_type_id
        and (d.year*100+d.month)>=v_current_ym
        and (
          d.rules_snapshot->>'source'='legacy_cutover'
          or coalesce((d.metrics->>'legacyCutover')::boolean,false)=true
          or dap.source='legacy_cutover'
        )
        and not exists(
          select 1 from public.dynamic_schedule_publications p
          where p.job_type_id=d.job_type_id and p.year=d.year and p.month=d.month
        )
      order by d.year,d.month,d.created_at desc nulls last,d.id desc
    loop
      v_source_period_id := nullif(coalesce(
        v_draft.metrics->>'sourcePeriodId',
        v_draft.config_snapshot->>'sourcePeriodId'
      ),'')::uuid;

      if v_map.source_kind='dispatcher' then
        select coalesce(p.status::text,'published') into v_source_status
        from public.schedule_periods p where p.id=v_source_period_id;
      elsif v_map.source_kind='on_call' then
        select coalesce(p.status::text,'published') into v_source_status
        from public.driver_schedule_periods p where p.id=v_source_period_id;
      else
        select coalesce(p.status::text,'published') into v_source_status
        from public.morning_driver_schedule_periods p where p.id=v_source_period_id;
      end if;
      v_source_status := coalesce(v_source_status,'published');
      ...
      insert into public.dynamic_schedule_publications(
        ...
        v_map.job_type_id,
        v_draft.availability_period_id,
        v_draft.draft_id,
        v_draft.year,
        v_draft.month,
        case when v_source_status='archived' then 'archived' else 'published' end,
        coalesce(v_draft.config_snapshot,'{}'::jsonb)
          || jsonb_build_object('legacyCutover',true,'migrationRunId',v_run.id,'repairedBy','phase9_1_2'),
        v_actor,
        now()
      ...
      where a.draft_id=v_draft.draft_id
      ...
      where o.draft_id=v_draft.draft_id and o.intentionally_unassigned_count>0
...
begin
  ...
  -- 6. Current/future published assignments must belong to an active member of the publication job type.
  -- Historical/archived assignments may legitimately reference users who are inactive today and must not block cutover.
  select count(*) into v_count
  ...
  where p.status='published'
    and (p.year*100+p.month) >= v_current_ym
    and (m.user_id is null or coalesce(pr.is_active,false)=false);
  if v_count=0 then
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'code','assignment_membership','status','pass','area','schedule','title','כל השיבוצים החיים שייכים לחברי תפקיד פעילים',
      'message','לא נמצאו שיבוצים בחודש הנוכחי או בעתיד לעובד ללא membership או לפרופיל לא פעיל.','count',0
    )); v_pass_count := v_pass_count + 1;
  else
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'code','assignment_membership','status','fail','area','schedule','title','נמצאו שיבוצים חיים עם שיוך משתמש לא תקין',
      'message','זהו blocker ל-Cutover: בחודש הנוכחי או בעתיד יש assignment לעובד ללא membership מתאים או לפרופיל לא פעיל.','count',v_count
    )); v_fail_count := v_fail_count + 1;
  end if;
