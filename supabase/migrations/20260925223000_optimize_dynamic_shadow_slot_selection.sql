-- Performance v4: cache required slot state and static scarcity.
-- Avoids invoking dynamic_shadow_candidate_allowed across every remaining slot on every loop iteration.
-- Dynamic eligibility is still checked when selecting the actual candidate, preserving scheduling rules.
...
begin
  ...
  set status=case when unfilled=0 then 'generated' else 'incomplete' end,
      metrics=jsonb_build_object(
        ...
        'algorithm','scarcity_first_cached_slot_state_balance_v2_5','wholePeriodScarcity',true,
