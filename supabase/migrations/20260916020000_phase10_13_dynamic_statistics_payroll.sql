...
-- Phase 10.13: explicit per-job-type statistics, meaningful shift-time grouping,
-- and generic projected payroll driven by the Job Type pay model/config.
-- Existing report-only/no-scheduling Daily Report roles are opted out automatically.
update public.job_types
set statistics_config=jsonb_set(coalesce(statistics_config,'{}'::jsonb),'{enabled}','false'::jsonb,true)
where scheduling_strategy='none'
  and coalesce((scheduling_config->'dailyReports'->>'enabled')::boolean,false)=true;
...
begin
  ...
  if not coalesce((target_job.statistics_config->>'enabled')::boolean,false) then
    raise exception 'statistics disabled for this job type';
  end if;
  ...
  raw_assignments as (
    ...
    select
      ...
      ha.starts_at::time as start_time,
      ha.ends_at::time as end_time,
  ...
  select jsonb_build_object(
    'jobType', jsonb_build_object(
      ...
      'payrollEnabled',coalesce(target_job.pay_model,'none')<>'none',
      'baseRate',coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0),
      'shiftRate',coalesce(nullif(target_job.pay_config->>'shiftRate','')::numeric,0)
    ),
    ...
    'shifts',coalesce((
      select jsonb_agg(jsonb_build_object(
        'shiftCode',s.time_key,
        'shiftName',s.time_label,
        'timeLabel',s.time_label,
        'assignmentCount',s.assignment_count,
        'timedHours',s.timed_hours
      ) order by s.start_time nulls last,s.end_time nulls last)
      from (
        select
          coalesce(to_char(start_time,'HH24:MI')||'-'||to_char(end_time,'HH24:MI'),'ללא שעות') time_key,
          coalesce(to_char(start_time,'HH24:MI')||'–'||to_char(end_time,'HH24:MI'),'ללא שעות') time_label,
          start_time,end_time,
          count(*)::integer assignment_count,
          coalesce(round(sum(timed_hours) filter(where timed_hours is not null),2),0) timed_hours
        from all_assignments
    ...
    'payrollPeople',coalesce((
      select jsonb_agg(jsonb_build_object(
        'userId',p.id,
        'displayName',p.display_name,
        'scheduleName',p.schedule_name,
        'assignmentCount',coalesce(x.assignment_count,0),
        'timedHours',coalesce(x.timed_hours,0),
        'workDayCount',coalesce(x.work_day_count,0),
        'projectedPay',coalesce(x.projected_pay,0)
      ) order by coalesce(x.projected_pay,0) desc,p.display_name)
      from candidate_users cu
      join public.profiles p on p.id=cu.user_id
      left join lateral (
        select count(*)::integer assignment_count,
          coalesce(round(sum(a.timed_hours) filter(where a.timed_hours is not null),2),0) timed_hours,
          count(distinct a.work_date)::integer work_day_count,
          round(case coalesce(target_job.pay_model,'none')
            when 'hourly' then coalesce(sum(a.timed_hours),0) * coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
            when 'per_shift' then count(*) * coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
            when 'per_day' then count(distinct a.work_date) * coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
            when 'mixed' then
              coalesce(sum(a.timed_hours),0) * coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
              + count(*) * coalesce(nullif(target_job.pay_config->>'shiftRate','')::numeric,0)
            else 0 end,2) projected_pay
        from all_assignments a where a.user_id=p.id
      ) x on true
    ),'[]'::jsonb),
    'payrollTotal',coalesce((
      select round(sum(case coalesce(target_job.pay_model,'none')
        when 'hourly' then coalesce(a.timed_hours,0) * coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
        when 'per_shift' then coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
        when 'per_day' then 0
        when 'mixed' then coalesce(a.timed_hours,0) * coalesce(nullif(target_job.pay_config->>'baseRate','')::numeric,0)
          + coalesce(nullif(target_job.pay_config->>'shiftRate','')::numeric,0)
        else 0 end),2) from all_assignments a
    ),0),
