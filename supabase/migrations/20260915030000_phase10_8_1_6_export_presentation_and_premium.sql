-- Phase 10.8.1.6
-- Keep Excel export presentation readable and resolve 200% metadata from the
-- effective Dynamic Job Type materialization when the concrete availability
-- slot did not persist those flags in metadata.
...
begin
  ...
  raw_slots as (
    ...
    left join lateral (
      select st.contains_200_percent, st.premium_200_hours
      ...
      where mat.job_type_id = ej.id
        and mat.effective_month <= month_start
