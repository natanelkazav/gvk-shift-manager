...
-- Phase 9.1: non-destructive pilot QA report.
  ...
  if v_latest_run.id is not null then
    ...
    loop
      v_reconciliation := v_reconciliation || jsonb_build_array(jsonb_build_object(
        ...
        'legacyCount',r.legacy_count,'dynamicAssignedCount',r.dynamic_count,
        'dynamicUnassignedCount',r.unassigned_count,
        'coveredCount',r.dynamic_count+r.unassigned_count,
        'matches',r.legacy_count=(r.dynamic_count+r.unassigned_count)
      ));
    end loop;
