-- Stronger availability-aware balancing plus 200% premium-hour fairness.
-- Shift-type fairness is intentionally based on the visible shift time, not shift_code,
-- so equivalent templates (for example weekday/special-day variants) compete in one pool.
...
returns numeric language sql stable security definer set search_path='' as $function$
  select coalesce(sum(case e.availability_status when 'preferred' then 1.25 when 'available' then 1.0 when 'avoid' then 0.15 else 0 end),0)::numeric
  ...
  join public.dynamic_availability_slots candidate_slot on candidate_slot.period_id=d.availability_period_id
   and candidate_slot.start_time=requested_slot.start_time and candidate_slot.end_time=requested_slot.end_time
...
returns numeric language sql stable security definer set search_path='' as $function$
  with requested as (
    select sl.start_time,sl.end_time,d.job_type_id
    ...
    where a.draft_id=requested_draft_id and sl.start_time=r.start_time and sl.end_time=r.end_time
  )
...
-- Hours inside a slot that are paid at 200% or above, from the period snapshot.
create or replace function public.dynamic_shadow_slot_200_hours(requested_slot_id uuid)
returns numeric language sql stable security definer set search_path='' as $function$
  select coalesce(sum(
    case when coalesce(nullif(seg->>'multiplier','')::numeric,1) >= 2
      then coalesce(nullif(seg->>'hours','')::numeric,0)
      else 0 end
  ),0)::numeric
  from public.dynamic_availability_slots sl
  left join lateral jsonb_array_elements(coalesce(sl.pay_segments_snapshot,'[]'::jsonb)) seg on true
  where sl.id=requested_slot_id;
$function$;

create or replace function public.dynamic_shadow_200_assigned_hours(requested_draft_id uuid, requested_user_id uuid)
returns numeric language sql stable security definer set search_path='' as $function$
  select coalesce(sum(public.dynamic_shadow_slot_200_hours(a.slot_id)),0)::numeric
  from public.dynamic_schedule_shadow_assignments a
  where a.draft_id=requested_draft_id and a.user_id=requested_user_id;
$function$;

create or replace function public.dynamic_shadow_200_opportunity_weight(requested_draft_id uuid, requested_user_id uuid)
returns numeric language sql stable security definer set search_path='' as $function$
  select coalesce(sum(
    public.dynamic_shadow_slot_200_hours(sl.id) *
    case e.availability_status when 'preferred' then 1.25 when 'available' then 1.0 when 'avoid' then 0.15 else 0 end
  ),0)::numeric
  from public.dynamic_schedule_shadow_drafts d
  join public.dynamic_availability_slots sl on sl.period_id=d.availability_period_id
  left join public.dynamic_availability_submissions sub on sub.period_id=d.availability_period_id and sub.user_id=requested_user_id
  left join public.dynamic_availability_entries e on e.submission_id=sub.id and e.slot_id=sl.id
  where d.id=requested_draft_id;
$function$;

create or replace function public.dynamic_shadow_200_expected_hours(requested_draft_id uuid, requested_user_id uuid, requested_slot_id uuid)
returns numeric language sql stable security definer set search_path='' as $function$
  with draft as (
    select d.job_type_id from public.dynamic_schedule_shadow_drafts d where d.id=requested_draft_id
  ), weights as (
    select m.user_id,public.dynamic_shadow_200_opportunity_weight(requested_draft_id,m.user_id) weight
    from draft d join public.job_type_memberships m on m.job_type_id=d.job_type_id
    join public.profiles p on p.id=m.user_id and p.is_active=true
  ), premium_total as (
    select coalesce(sum(public.dynamic_shadow_slot_200_hours(a.slot_id)),0)::numeric
           + public.dynamic_shadow_slot_200_hours(requested_slot_id) hours
    from public.dynamic_schedule_shadow_assignments a where a.draft_id=requested_draft_id
  )
  select case when coalesce(sum(weight),0)<=0 then 0 else (select hours from premium_total)
    * coalesce(max(weight) filter(where user_id=requested_user_id),0) / sum(weight) end from weights;
$function$;
...
grant execute on function public.dynamic_shadow_slot_200_hours(uuid) to authenticated;
grant execute on function public.dynamic_shadow_200_assigned_hours(uuid,uuid) to authenticated;
grant execute on function public.dynamic_shadow_200_opportunity_weight(uuid,uuid) to authenticated;
grant execute on function public.dynamic_shadow_200_expected_hours(uuid,uuid,uuid) to authenticated;
...
begin
    ...
    from (
      select t.user_id,t.assigned_count,t.requested_min,t.requested_target,t.requested_max,e.availability_status,
        ...
        + (public.dynamic_shadow_shift_type_expected_count(v_draft_id,t.user_id,sl.id)
           - public.dynamic_shadow_shift_type_assigned_count(v_draft_id,t.user_id,sl.id))*fairness_weight*1.5
        + (public.dynamic_shadow_200_expected_hours(v_draft_id,t.user_id,sl.id)
           - public.dynamic_shadow_200_assigned_hours(v_draft_id,t.user_id))*fairness_weight*0.45
  ...
  set status=case when unfilled=0 then 'generated' else 'incomplete' end,
      metrics=jsonb_build_object(
        ...
        'algorithm','scarcity_first_availability_type_premium_balance_v2_3','wholePeriodScarcity',true,
