...
-- Phase 10.5.8 · Unified management schedule calendar.
...
begin
  ...
  live_slots as (
    select
      ...
      greatest(
        coalesce(sl.target_workers, 1) - coalesce(assignment_data.assigned_count, 0),
        coalesce(pu.intentionally_unassigned_count, 0),
        0
      )::integer as unassigned_count,
      coalesce(sl.target_workers, 1)::integer as required_count
    ...
    left join public.dynamic_schedule_published_unassigned pu
      on pu.publication_id = pub.id and pu.slot_id = sl.id
    left join lateral (
      select
        ...
        coalesce(
          jsonb_agg(
            jsonb_build_object(
              'userId', a.user_id,
              'displayName', coalesce(p.display_name, 'משתמש לא פעיל')
            ) order by coalesce(p.display_name, 'משתמש לא פעיל')
