...
type SchedulingHubView = 'management' | 'boards' | 'calendar';
...
function ShiftsPage() {
  ...
  const loadCalendarWorkspace = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setCalendarError(loadError instanceof Error ? loadError.message : 'טעינת לוח השנה המאוחד נכשלה.');
    } finally {
    ...
    <div className="dynamic-shifts-page">
      ...
      <div className="dynamic-shifts-hub-tabs" role="tablist" aria-label="תצוגת מרכז השיבוצים">
        ...
        <button
          type="button"
          role="tab"
          aria-selected={hubView === 'calendar'}
          className={hubView === 'calendar' ? 'is-active' : ''}
          onClick={openCalendarView}
        >
          <CalendarDays size={18} />
          <span>לוח שנה</span>
          <small>החודש הנוכחי והיסטוריה לפי תפקיד ואיוש</small>
        </button>
      </div>
      ...
      <div className="dynamic-shifts-monthbar" aria-label="בחירת חודש">
        ...
        <div className="dynamic-shifts-month-title">
          ...
          <div>
            ...
            <span>{hubView === 'boards'
              ? 'כל הלוחות שפורסמו בחודש הנבחר'
              : hubView === 'calendar'
                ? 'לוח מאוחד של כל השיבוצים בחודש הנבחר'
                : 'כל התפקידים המנוהלים בחודש הנבחר'}</span>
          </div>
        </div>
        <Button
          variant="secondary"
          onClick={() => changeMonth(1)}
          disabled={hubView === 'calendar' && (year * 100 + month) >= (initial.year * 100 + initial.month)}
        >
      ...
      {loading ? (
        ...
        hubView === 'management' ? (
          ...
          <section className="dynamic-shifts-boards-section" aria-labelledby="all-schedules-calendar-title">
            <div className="dynamic-shifts-section-head">
              <div>
                <h2 id="all-schedules-calendar-title">לוח שנה — כל השיבוצים</h2>
                <p>תצוגת מנהלים מאוחדת של החודש הנוכחי וחודשים קודמים, כולל שם העובד ופילטר לתפקידים ולשיבוצים לא מאוישים.</p>
              </div>
              <Button variant="secondary" disabled={calendarLoading} onClick={() => void loadCalendarWorkspace()}>
                <RefreshCw size={16} className={calendarLoading ? 'spin' : undefined} /> רענן לוח
              </Button>
            </div>
            ...
            {calendarLoading ? (
              <div className="dynamic-shifts-loading"><LoaderCircle className="spin" size={22} /> טוען לוח שנה מאוחד…</div>
            ) : calendarWorkspace ? (
              <DynamicAllSchedulesCalendar workspace={calendarWorkspace} />
            ) : (
              <div className="dynamic-shifts-empty">
                <CalendarDays size={24} />
                <div>
                  <strong>אין נתוני לוח להצגה</strong>
                  <span>בחר חודש נוכחי או חודש קודם ורענן את הלוח.</span>
                </div>
              </div>
            )}
          </section>
