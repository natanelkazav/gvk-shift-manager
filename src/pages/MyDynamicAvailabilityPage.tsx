  ...
  return <main className="my-dynamic-availability page-shell" dir="rtl">
    ...
    {!periods.length?<section className="my-availability-empty"><h2>אין כרגע תקופת אילוצים זמינה</h2><p>כאשר מנהל יפתח תקופה לתפקיד שאליו אתה משויך, היא תופיע כאן.</p></section>:<>
      ...
      {workspace&&selected?<section className="my-availability-card">
        ...
        <div className="my-availability-list">{workspace.slots.map(slot=>{const currentStatus=statusBySlot[slot.id]??'';return <div className={`my-availability-row ${statusClass(currentStatus)}`} key={slot.id}><div><strong>{dynamicShiftDisplayName(slot.shiftName, 'משמרת')}</strong><div className="my-availability-slot-meta"><span className="my-availability-date" dir="ltr">{formatDate(slot.date)}</span><span aria-hidden="true">·</span><span>{weekdays[dayOf(slot.date)]}</span><span aria-hidden="true">·</span><span className="my-availability-time" dir="ltr">{slot.startTime.slice(0,5)}–{slot.endTime.slice(0,5)}</span></div>{slot.holidayName?<small>{slot.holidayName}</small>:null}</div><select className={`my-availability-status-select ${statusClass(currentStatus)}`} disabled={!editable} value={currentStatus} onChange={e=>setStatusBySlot(cur=>({...cur,[slot.id]:e.target.value as Status}))}><option value="">לא סומן</option>{statuses.map(s=><option key={s} value={s}>{labels[s]}</option>)}</select></div>})}</div>
        {editable?<div className={`my-availability-footer${alreadySubmitted?' is-submitted':''}`}><div className="my-availability-footer-status">{alreadySubmitted?<><strong><CheckCircle2 size={18}/> האילוצים הוגשו</strong><span>{selected.submittedAt?`הוגש ב-${new Date(selected.submittedAt).toLocaleString('he-IL')}`:'ניתן לעדכן את ההגשה כל עוד התקופה פתוחה.'}</span></>:<span>{filled<total?`נותרו ${total-filled} משמרות לסימון.`:'כל המשמרות סומנו ומוכנים להגשה.'}</span>}</div><div>{alreadySubmitted?null:<Button variant="secondary" disabled={busy} onClick={()=>void save(false)}>שמור טיוטה</Button>}<Button disabled={busy||filled<total} onClick={()=>void save(true)}>{alreadySubmitted?'עדכן הגשה':'הגש אילוצים'}</Button></div></div>:null}
      </section>:null}
    </>}
  </main>;
}
